...
def getCommonSense(utterance, path):
	command = ('{} comet-commonsense/scripts/generate/generate_conceptnet_arbitrary.py --model_file comet-commonsense/pretrained_models/conceptnet_pretrained_model.pickle --input "{}" --output_file output.json --device 0 --sampling_algorithm beam-5').format(
		path, utterance)
	os.system(command)
	...
	try:
		get_beams = output[0]['Causes']['beams']
	except:
		get_beams = []
	return get_beams


def retrieveCommonSense(utterance, path):
	modified_utterance = preprocess(utterance)
	return getCommonSense(modified_utterance, path)
