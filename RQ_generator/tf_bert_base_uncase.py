...
from __init__ import BertBaseUnCaseV2, PATH_PIC_RQ, PSD_VERSION, TrainModelConfigV2, PATH_TRANS_INPUT


if not BertBaseUnCaseV2.SHOW_MODE:
...
def merge_response_context(input, model_type):
    ...
    only for bert.
    roberta special token is <cls> and <sep>
    """
...
def triple_label_v2(df, model_type):
    ...
    """
    df['tri_label'] = df[['sarcasm', 'question', 'rq']].apply(merge_label, axis=1)
    df['context_merge'] = df[['context_2', 'context_1', 'response']].apply(merge_response_context, args=(model_type,),
                                                                           axis=1)
    return df[['context_merge', 'tri_label']]


def triple_label_v3(df, model_type):
    """
    use the splited response and contexts
    :param df:
    :return: tri_label only for rq and literal
    """
    df['tri_label'] = df[['sarcasm', 'question', 'rq']].apply(merge_label, axis=1)
    df = df.loc[df['tri_label'] != 2]
    df['context_merge'] = df[['context_2', 'context_1', 'response']].apply(merge_response_context, args=(model_type,),
                                                                           axis=1)
    return df[['context_merge', 'tri_label']]
...
def convert_example_to_feature(review, tokenizer, model_type):
    ...
    if model_type in TrainModelConfigV2.BERT_LIST:
        return tokenizer.encode_plus(review,
                                     ...
                                     max_length=BertBaseUnCaseV2.MAXLEN,  # max length of the text that can go to BERT
        ...
        return tokenizer.encode_plus(review,
                                     ...
                                     max_length=BertBaseUnCaseV2.MAXLEN,
                                     padding='max_length',
...
def encode_examples_roberta(ds, tokenizer, model_type, train=True):
    input_ids_list = []
    # token_type_ids_list = np.zeros((ds.shape[0], BertBaseUnCaseV2.MAXLEN), dtype='int32').tolist()
    attention_mask_list = []
...
class BuildModels:
    ...
    def build_model_1(self, verbose=False):
        ...
        if self.model_type in TrainModelConfigV2.BERT_LIST:
            ...
            input_ids_layer = Input(shape=(BertBaseUnCaseV2.MAXLEN,), dtype=np.int32, name='input_ids')
            input_mask_layer = Input(shape=(BertBaseUnCaseV2.MAXLEN,), dtype=np.int32, name='attention_mask')
            input_token_type_layer = Input(shape=(BertBaseUnCaseV2.MAXLEN,), dtype=np.int32, name='token_type_ids')
        ...
        if self.version == "v1":
            ...
            flat_layer = Flatten()(bert_layer)
            dense_layer = Dense(BertBaseUnCaseV2.hidden_size, activation='relu')(flat_layer)
            out = Dropout(0.2)(dense_layer)
        elif self.version == "v4":
            bi_layer = Bidirectional(LSTM(BertBaseUnCaseV2.hidden_size, dropout=0.2, return_sequences=True))(bert_layer)
            bi_layer = Bidirectional(LSTM(BertBaseUnCaseV2.hidden_size))(bi_layer)
            dropout_layer = Dropout(0.2)(bi_layer)
            out = Dense(256, activation='relu')(dropout_layer)


        if BertBaseUnCaseV2.VER == 'v5':
            dense_output = Dense(BertBaseUnCaseV2.N_CLASS, activation='sigmoid')(out)
        else:
            dense_output = Dense(BertBaseUnCaseV2.N_CLASS, activation='softmax')(out)
...
def train_model(model, train, val, test_count, model_type, checkpoint_path):
    """training model"""
    cp_path = '{}{}{}.h5'.format(checkpoint_path, BertBaseUnCaseV2.m_ver, test_count)
    ckpt = CustomModelCheckpoint(model, cp_path, verbose=BertBaseUnCaseV2.SHOW_MODE_NUM)

    es = EarlyStopping(monitor='val_loss', patience=2, verbose=BertBaseUnCaseV2.SHOW_MODE_NUM)
    bert_his = model.fit(train, epochs=BertBaseUnCaseV2.EPOCHS, validation_data=val, callbacks=[ckpt, es],
                         verbose=BertBaseUnCaseV2.SHOW_MODE_NUM)
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig('{}{}-{}{}{}{}.png'.format(PATH_PIC_RQ, model_type, PSD_VERSION, BertBaseUnCaseV2.VER,
                                           BertBaseUnCaseV2.m_ver, test_count))
    plt.clf()
    plt.close('ALL')
